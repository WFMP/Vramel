...
/**
 * A sized {@link java.util.concurrent.ScheduledExecutorService} which will reject executing tasks if the task queue is full.
 * <p/>
 * The {@link java.util.concurrent.ScheduledThreadPoolExecutor} which is the default implementation of the {@link java.util.concurrent.ScheduledExecutorService}
 * has unbounded task queue, which mean you can keep scheduling tasks which may cause the system to run out of memory.
 * <p/>
 * This class is a wrapped for {@link java.util.concurrent.ScheduledThreadPoolExecutor} to reject executing tasks if an upper limit
